...
try:
    ...
    print("Pandas module does not exist in python path, but is needed for chem_aerosol_tables.")
...
def chem_aerosol_tables(adf):
    '''
    
    '''

    #CAM simulation variables (these quantities are always lists):
    case_names = adf.get_cam_info('cam_case_name', required=True) + [adf.get_baseline_info("cam_case_name")]
    #case_names_len = len(case_names)

    res = adf.variable_defaults # dict of variable-specific plot preferences

    start_yrs = adf.climo_yrs["syears"] + [adf.climo_yrs["syear_baseline"]]
    end_yrs = adf.climo_yrs["eyears"] + [adf.climo_yrs["eyear_baseline"]]
  
    #Grab history file number strings:
    cam_hist_strs = adf.hist_string["test_hist_str"]
    hist_strs = cam_hist_strs + [adf.hist_string["base_hist_str"]]
    ...
    hist_locs = cam_hist_locs + [adf.get_baseline_info("cam_hist_loc")]
    ...
    for case_idx,case in enumerate(case_names):
        ...
        adf.debug_log(f"DEBUG: location of files is {str(hist_locs[case_idx])}")
    ...
    for i,case in enumerate(case_names):
        ...
        #print("WOOO",Path(data_dirs[i]) / Files[0])
    ...
    if len(AEROSOL_VARIABLES) > 0:
        ...
        make_table(AEROSOL_VARIABLES,'aerosols',Dic_scn_var_comp,areas)

    if len(GAS_VARIABLES) > 0:
        print("\tMaking table for gases")
        make_table(GAS_VARIABLES,'gases',Dic_scn_var_comp,areas)
...
def make_Dic_scn_var_comp(variables, data_dir, dic_SE):
    ...
    for ivar,current_var in enumerate(variables):
        ...
        for comp in components:
            ...
            current_data,missing_vars,needed_vars=SEbudget(dic_SE,current_dir,current_files,comp)
    ...
    # Just run this once
    current_crit=SEbudget(dic_SE,current_dir,current_files,'O3')
...
def calc_budget_data(case, current_var, Dic_scn_var_comp, area):
    ...
    if 'AOD' in current_var:
        ...
        chem_dict[case][f"{current_var}_mean"] = np.round(BURDEN,5)
    else:
        # Surface Emissions
        ...
        chem_dict[case][f"{current_var}_BURDEN (Tg{specifier})"] = np.round(BURDEN,5)
        ...
        if current_var == 'SO4': # chemical production is basically the elevated emissions.
            ...
            chem_dict[case][f"{current_var}_CHEM_PROD (Tg{specifier}/yr)"] = 0
        else:
        ...
        if current_var in AEROSOLS:
           ...
           # Dry Deposition Flux
            ...
            # Total Deposition
            TDEP = DDF - WDF
            chem_dict[case][f"{current_var}_TDEP (Tg/yr)"] = np.round(TDEP,5)

            # LifeTime = Burden/(loss+deposition)
            #print("LifeTime")
            if current_var == "CH4":
                LT = BURDEN/(CHML+DDF-WDF) # years
                chem_dict[case][f"{current_var}_LIFETIME (years)"] = np.round(LT,5)
            else:
                if (CHML+DDF-WDF) > 0:
                    if CHML != 0:
                        LT = BURDEN/(CHML+DDF-WDF)*duration/86400/num_yrs[0] # days
                        ...
                        chem_dict[case][f"{current_var}_LIFETIME (days)"] = 0
            ...
            if current_var == "O3":
                ...
                STE = DDF-TND
                chem_dict[case][f"{current_var}_STE (Tg/yr)"] = np.round(STE,5)
