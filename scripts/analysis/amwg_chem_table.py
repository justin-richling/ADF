...
def amwg_chem_table(adf):
    ...
    thing_ext_list_O3 = {'_CHMP':'_CHEM_PROD','_NET':'_NET',
                        '_DDF':'_DRYDEP','_WDF':'_WETDEP',
                        '_STE':'_STE'}

    thing_ext_list_O3_full = {**thing_ext_list_main, **thing_ext_list_O3}
    ...
    aerosols_ext_dict = {'_BURDEN':'_BURDEN','_CHMP':'_CHEM_PROD','_SF':'_EMIS',
...
def list_files(directory,scenario,start_date,end_date):
    ...
    all_fileNames=[]
    print("all_filenames[0][0:scenario_len]",all_filenames[0][11-scenario_len:-11])
    for i in range(len(all_filenames)):
        if all_filenames[i][11-scenario_len:-11]==scenario: # check if the file is relevant
