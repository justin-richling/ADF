...
def aerosol_gas_tables(adfobj):
    ...
    for i,case in enumerate(case_names):
        ...
        print(f"hist_str for case {case}",hist_str,"\n")
...
def list_files(adfobj, directory, hist_str, start_year ,end_year):#, h_case
    ...
    test_files = sorted(Path(directory).glob(f'*.{hist_str[0]}.{yrs[0]}-*.nc'))
    print("test_files",test_files,"\n")
    all_filenames = []
    for i in yrs:
        all_filenames.append(sorted(Path(directory).glob(f'*.{hist_str[0]}.{i}-*.nc')))
