"""
Module to create flight campaign composited data.
"""


def create_composited_dataset(composited_data_path, campaign):
    """
    ...
    import compass_utils as compass # INFORM python module inform_utils.py
    ...
    df = compass.load_flight_data(dir,flight_number-1) # 0 is Index of research flight

    blocks = compass.VAP_process_flight_data(df,5)
    dict = compass.assign_flight_type(df)
    compass.plot_block_ts(dict,5)


    # Select ERA5 dataset for Aircraft Campaign Time Frame
    ds = compass.select_ERA5_4flight(df) 
    # Collocate ERA5 data and calculate environmental controlling factors
    fblks_coll = compass.collocate_ERA5_dat(ds, blocks)
    # Select cloud regime type based on cloud controlling factors
    fblks_cr = compass.cloud_regime_old(fblks_coll)

    rf_id = f"RF{flight_number:02d}"
    compass.write_RF_nc(fblks_cr, rf_id)
"""


    #campaign = 'SOCRATES' # SOCRATES or CSET
    #composited_data_path = '/glade/derecho/scratch/richling/inform/composited_data'
    #composited_data_path = composited_data
    if campaign == 'SOCRATES':
        dir = '/glade/work/patnaude/SOCRATES/aircraft_1hz/'
    elif campaign  == 'CSET':
        dir = '/glade/work/patnaude/CSET/aircraft_1hz/'
            
    flight_paths = compass.find_flight_fnames(dir)
    all_fblks = {}

    for i in tqdm(range(0,len(flight_paths))):
        df = compass.load_flight_data(dir, i)
        ...
        blocks = compass.VAP_process_flight_data(df, i)
        dict = compass.assign_flight_type(df)
        compass.plot_block_ts(dict,5)

        # Select ERA5 data
        ds = compass.select_ERA5_4flight(df, campaign)
        rf_id = f"RF{i+1:02d}"
        print(rf_id)
        ...
        fblks_coll = compass.collocate_ERA5_dat(ds, blocks)
        # Select cloud regime type based on cloud controlling factors
        fblks_cr = compass.cloud_regime(fblks_coll, campaign=campaign)
        # Write to NetCDF for this flight
        compass.write_RF_nc(fblks_cr, rf_id, campaign, save_path=composited_data_path)
