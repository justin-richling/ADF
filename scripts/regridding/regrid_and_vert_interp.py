...
def regrid_and_vert_interp(adf):
    ...
    if adf.compare_obs:
        ...
        bl_time_string = f"{str(bl_syr).zfill(4)}01-{str(bl_eyr).zfill(4)}12"
    #End if
    ...
    for case_idx, case_name in enumerate(case_names):
        ...
        time_string = f"{str(start_year[case_idx]).zfill(4)}01-{str(end_year[case_idx]).zfill(4)}12"
        ...
        for var in var_list:
            ...
            for target in target_list:
                ...
                if not regridded_file_loc.is_file():
                    ...
                    mclim_fils = sorted(mclimo_loc.glob(f"{case_name}_{var}_climo_{time_string}.nc"))
