...
def meridional_mean(adfobj):
    ...
    **Note:** the constraint of 5S to 5N is easily changed;
    ...
    for var in var_list:
        ...
        validate_ref_lat_lev = utils.validate_dims(odata, ['lat', 'lev'])

        #Check if reference file has vertical levels
        #Notify user of level dimension:
        if validate_ref_lat_lev['has_lev']:
            print(f"\t    INFO: {var} has lev dimension.")
            has_lev_ref = True
        else:
            has_lev_ref = False

        print("validate_ref_lat_lev['has_lon']",validate_ref_lat_lev['has_lon'])
        ...
        for case_idx, case_name in enumerate(adfobj.data.case_names):
            ...
            validate_lat_lev = utils.validate_dims(mdata, ['lat', 'lev'])
            ...
            for s in seasons:
                ...
                if plot_name not in meridional_skip:
                    ...
                    pf.plot_meridional_mean_and_save(plot_name, case_nickname, adfobj.data.ref_nickname,
                                                    ...
                                                    mseasons[s], oseasons[s], has_lev, latbounds=slice(-5,5),
                                                    log_p=False, obs=adfobj.compare_obs, **vres)
                ...
                if (plot_name_log) and (plot_name_log not in logp_meridional_skip):
                    ...
                    pf.plot_meridional_mean_and_save(plot_name_log, case_nickname, adfobj.data.ref_nickname,
                                                        ...
                                                        [syear_baseline,eyear_baseline],
                                                        mseasons[s], oseasons[s], has_lev, latbounds=slice(-5,5),
                                                        log_p=True, obs=adfobj.compare_obs, **vres)
