...
def tem_plot(ds, ds_base, case_names, axs, s, var_list, res, obs, climo_yrs):
    ...
    for var in var_list:
        ...
        if len(dseasons.lev) == 0:
            ...
            img2 = axs[axs_id,2].contourf(lats,levs,dseasons, cmap="BrBG",levels=15,norm=cp_info['normdiff'])#levels=levs_diff
            axs[axs_id,2].contour(lats,levs,dseasons, colors="k",levels=15,norm=cp_info['normdiff'])#levels=diff_levs[::2]
