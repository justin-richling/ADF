...
def tem_plot(ds, ds_base, case_names, axs, s, var_list, res, obs, climo_yrs):
    ...
    for var in var_list:
        ...
        #------------------------------------------------------------------------------------------
        if var == "epfy":
            mseasons.plot(ax=axs[2,0], y='lev', yscale='log',ylim=[1e2,1],levels=np.arange(-5e6,5.1e6,0.5e6),
                                    cbar_kwargs={'label': ds[var].units},cmap="RdYlBu_r")
            mseasons.plot.contour(ax=axs[2,0], y='lev', yscale='log',
                                                ylim=[1e2,1],levels=np.arange(-5e6,5.1e6,1e6),
                                                colors='black', linestyles=None)

            oseasons.plot(ax=axs[2,1], y='lev', yscale='log',ylim=[1e2,1],levels=np.arange(-5e6,5.1e6,0.5e6),
                                    cbar_kwargs={'label': ds[var].units},cmap="RdYlBu_r")
        ...
        if var == "epfz":
            mseasons.plot(ax=axs[3,0], y='lev', yscale='log',ylim=[1e2,1],levels=np.arange(-3e4,3.1e4,0.25e4),
                                    cbar_kwargs={'label': ds[var].units,'format': '%.0e'},cmap="RdYlBu_r",vmax=3e4)
        ...
        if var == "wtem":
            mseasons.plot.contourf(ax=axs[5,0], levels = np.arange(-5e-3,5.1e-3,0.5e-3), y='lev', yscale='log',
                                                ...
                                                cbar_kwargs={'label': ds[var].units,'format': '%.0e'},cmap="RdYlBu_r")
            ...
            if len(dseasons.lev) == 0:
                ...
                dseasons.plot(ax=axs[5,2], y='lev', yscale='log',
                            ylim=[1e2,1],cmap="BrBG",levels=np.arange(-0.002,0.002,0.0001),
                                    cbar_kwargs={'label': ds[var].units,'format': '%.0e'})
