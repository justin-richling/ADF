...
def tem_plot(ds, ds_base, case_names, axs, s, var_list, res, obs, climo_yrs):
    ...
    for var in var_list:
        ...
        #------------------------------------------------------------------------------------------
        if var == "epfy":
            mseasons.plot(ax=axs[2,0], y='lev', yscale='log',vmax=1e6,ylim=[1e2,1],levels=np.arange(-1e6,1.1e6,0.1e6),
                                    cbar_kwargs={'label': ds[var].units},cmap="RdYlBu_r")
            mseasons.plot.contour(ax=axs[2,0], y='lev', yscale='log',
                                                ylim=[1e2,1],levels=np.arange(-1e6,1.1e6,0.2e6),
                                                colors='black', linestyles=None)

            oseasons.plot(ax=axs[2,1], y='lev', yscale='log',vmax=1e6,ylim=[1e2,1],levels=np.arange(-1e6,1.1e6,0.1e6),
                                    cbar_kwargs={'label': ds[var].units},cmap="RdYlBu_r")
