...
def tape_recorder(adfobj):
    ...
    for var in ["Q"]:
        ...
        if multi_case:
            plot_name_multi = main_site_assets_path / f"tape_recorder_{var}_Special_multi_plot.{plot_type}"
            ...
            plot_name = plot_location / f"tape_recorder_{var}_Special.{plot_type}"
        ...
        for idx,key in enumerate(runs_LT2):
            ...
            if multi_case and key != base_nickname:
                ...
                #Save image
                plot_name = Path(plot_location[idx]) / f"tape_recorder_{var}_Special.{plot_type}"
