...
def time_series(adfobj):
    ...
    for season in seasons:
        ...
        for var in ts_var_list:
            ...
            adfobj.add_website_data(plot_name, var, category=web_category, season=season, plot_type="TimeSeries",multi_case=True)
            #Close plots:
            plt.close()
            ...
            for season in seasons:
                ...
                for var in ts_var_list:
                    ...
                    for case_idx, case_name in enumerate(case_names):
                        ...
                        plt.savefig(plot_name, facecolor='w')
                        #Add plot to website (if enabled):
                        adfobj.add_website_data(plot_name, var, category=web_category, season=season, plot_type="TimeSeries")
                        #Close plots:
                        plt.close()
                    #End for (case loop)
                #End for (variables loop)
