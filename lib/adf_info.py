...
class AdfInfo(AdfConfig):
    ...
    def get_climo_yrs_from_ts(self, input_ts_loc, case_name):
        ...
        var_list_num = 0
        ts_files = sorted(input_location.glob(f"{case_name}*.{var_list[var_list_num]}.*nc"))

        #Read in file(s)
        #If the first variable in the list isn't actually available, check checking until one is
        if len(ts_files) == 0:
            var_list_num += 1
            ts_files = sorted(input_location.glob(f"{case_name}*.{var_list[var_list_num]}.*nc"))"""


        #Search for first variable in var_list to get a time series file to read climo years
        #NOTE: it is assumed all the variables have the same dates!
        """def find_existing_file(var_list):
            for var in var_list:
                # Get all files matching the pattern
                ts_files = sorted(input_location.glob(f"{case_name}*.{var}.*nc"))
                # If any files are found, return the first one
                if ts_files:
                    return ts_files  # Return the first match

            # If no matches found
            return None

        # Find the first existing file
        ts_files = find_existing_file(var_list)"""

        for var in var_list:
            # Get all files matching the pattern
            print("AHAHAHA:",f"{case_name}*.{var}.*nc")
            ts_files = sorted(input_location.glob(f"{case_name}*.{var}.*nc"))
            # If any files are found, return the first one
            if ts_files:
                break
        ...
        if len(ts_files) == 0:
            print("No valid time series files to extract dates, please check your config file and/or CAM output")
