...
def make_polar_plot(wks, case_nickname, base_nickname,
    ...
    if len(levs_pctdiff) < 2:
        ...
        try:
            img3 = ax3.contourf(pct_cyclic.lon.values, pct_cyclic.lat.values, pct_cyclic, transform=ccrs.PlateCarree(), cmap=cmappct, norm=pctnorm, levels=levelspctdiff,transform_first=True)#, transform_first=True
        except TypeError:
            print("Here?")
            img3 = ax3.contourf(lons, lats, pct_cyclic, transform=ccrs.PlateCarree(), colors="w", norm=pctnorm)
            ax3.text(0.4, 0.4, empty_message, transform=ax3.transAxes, bbox=props)
