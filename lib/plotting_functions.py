...
def plot_map_and_save(wks, case_nickname, base_nickname,
    ...
    if kwargs["save_plot_data"]:
        ...
        data_array1 = xr.DataArray(wrap_fields[0], dims=("lat", "lon"), coords={"lat": lat, "lon": lon},name="test")
        data_array2 = xr.DataArray(wrap_fields[1], dims=("lat", "lon"), coords={"lat": lat, "lon": lon},name="ref")
        data_array3 = xr.DataArray(wrap_fields[2], dims=("lat", "lon"), coords={"lat": lat, "lon": lon},name="diff")
        combined = xr.Dataset({
            "test": data_array1,
            "ref": data_array2,
            "diff": data_array3
        ...
        if isinstance(combined, xr.Dataset):
            ...
            enc_dv = {}
        #End if
        enc_c = {xname: {'_FillValue': None} for xname in combined.coords}
        enc = {**enc_c, **enc_dv}
...
def plot_zonal_mean_and_save(wks, case_nickname, base_nickname,
    ...
    if has_lev:
        ...
        fig.legend(handles=[line,line2],#bbox_to_anchor=(-0.15, 0.87, 1.05, .102),
                    loc="upper left", borderaxespad=0.0,fontsize=6,frameon=False)
