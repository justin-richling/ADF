...
class AdfInfo(AdfConfig):
    ...
    def __init__(self, config_file, debug=False):
        ...
        #    pass
            ...
            input_ts_bl = self.get_baseline_info("cam_ts_loc")
            self.__bl_ts_locs = input_ts_bl
            ...
            calc_bl_ts   = self.get_baseline_info("cam_calc_ts")
            if (not calc_bl_ts) and (input_ts_bl):
                bl_ts_done = True # Premade TS files
            self.__bl_ts_done = bl_ts_done
            ...
            self.__calc_bl_ts = {}
            self.__calc_bl_ts[data_name] = calc_bl_ts

            #Check if any time series files are pre-made
            #bl_overwrite_ts   = self.get_baseline_info("cam_overwrite_ts")
            print("bl_overwrite_ts",bl_overwrite_ts,"\n")
            if bl_overwrite_ts is None:
                bl_overwrite_ts = False
            self.__bl_overwrite_ts = bl_overwrite_ts
            #self.__bl_overwrite_ts = {data_name:bl_overwrite_ts}

            input_climo_bl = self.get_baseline_info("cam_climo_loc")
            self.__bl_climo_locs = input_climo_bl
            #if not input_climo_bl:
            #    print()

            #Check if any climo files are pre-made
            #bl_overwrite_ts   = self.get_baseline_info("cam_overwrite_ts")
            bl_overwrite_climo   = self.get_baseline_info("cam_overwrite_climo")
            print("bl_overwrite_climo",bl_overwrite_climo,"\n")
            if bl_overwrite_climo is None:
                bl_overwrite_climo = False
            self.__bl_overwrite_climo = bl_overwrite_climo

            #if (bl_ts_done) and (not input_climo_bl):
            if (not calc_bl_ts) and (not input_climo_bl):
                self.__calc_bl_climo = False
            #if (baseline_ts_done) and (input_climo_bl):
            #    self.__calc_bl_climo = True
        ...
        if self.__bl_ts_done:
            bl_ts_done = self.__bl_ts_done
        else:
            bl_ts_done = False
    ...
    def calc_ts(self):
        return {"test": copy.copy(self.__calc_test_ts),
                "baseline": copy.copy(self.__calc_bl_ts)}
