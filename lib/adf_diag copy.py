...
class AdfDiag(AdfWeb):
    ...
    def derive_variables_xarray(self, res=None, vars_to_derive=None, ts_dir=None, overwrite=None):
        ...
        for var in vars_to_derive:
            ...
            equation_str = "x**2 + y**2 + z**2"

            # Create xarray Datasets with multidimensional variables
            variables = {'x': ('lat', 'lon', 'time'), 'y': ('lat', 'lon', 'time'), 'z': ('lat', 'lon', 'time')}
            data_arrays = []

            for var, dims in variables.items():
                print(dims)
                values = np.random.rand(5, 5, 10)  # Example data with dimensions time x space
                da = xr.DataArray(values, coords={'lat': np.arange(5), 'lon': np.arange(5), "time": np.arange(10)}, dims=dims)
                data_arrays.append(da)

            # Create xarray Dataset from DataArrays
            #ds = xr.Dataset(dict(zip(variables.keys(), data_arrays)))
            ...
            def sympy_function(l, s, t):
                return numeric_function(l, s, t)

            # Apply the symbolic function to the xarray Dataset
            result_ds = xr.apply_ufunc(sympy_function, *data_arrays, dask='parallelized', output_dtypes=[float])
