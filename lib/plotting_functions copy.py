    ...
    if ((adfobj.compare_obs) and (var in adfobj.var_obs_dict)) or (not adfobj.compare_obs):
        ...
        for season in multi_dict_var[case_names[0]].keys():
            ...
            if (not redo_plot) and Path(wks / file_name).is_file():
                ...
                fig_width, fig_height = calculate_figsize_from_width_ratios(
                        width_ratios, num_plot_columns=3,
                        plot_width=8,  # desired width of each plot
                        num_rows=nrows,
                        height_per_row=3
                    )
                ...
                for r in range(nrows):
                    ...
                    # generate dictionary of contour plot settings:
                    print(case_names[r], season, ptype,"\n")
                    print("multi_dict_var[case_names[r]][season]",multi_dict_var[case_names[r]][season][ptype],"\n\n")
                    mdlfld = multi_dict_var[case_names[r]][season][ptype]["m_data"]
                    mdlfld = mdlfld.sel(lat=slice(domain[2],domain[3]))
                    obsfld = multi_dict_var[case_names[r]][season][ptype]["o_data"]
                    obsfld = obsfld.sel(lat=slice(domain[2],domain[3]))
                    diffld = multi_dict_var[case_names[r]][season][ptype]["diff_data"]
                    diffld = diffld.sel(lat=slice(domain[2],domain[3]))
                    #pctld = multi_dict_var[case_names[r]][season]
                    cp_info = plot_utils.prep_contour_plot(mdlfld, obsfld, diffld, **kwargs)
                    ...
                    for c, key in enumerate(["m_data", "o_data", "diff_data"]):
                        ...
                        if key == "diff_data":
                            ...
                            #norm = cp_info['normdiff']
                            # Plot to the correct subplot
                            cf = ax.contourf(lons, lats, fld, levels=levels, cmap=cmap,
                                            norm=norm)
                        else:
                            adata = multi_dict_var[case_names[r]][season][ptype]["m_data"]
                            adata = adata.sel(lat=slice(domain[2],domain[3]))
                            lat = adata['lat']
                            adata, lon = add_cyclic_point(adata, coord=adata['lon'])
                            lons, lats = np.meshgrid(lon, lat)

                            bdata = multi_dict_var[case_names[r]][season][ptype]["o_data"]
                            bdata = bdata.sel(lat=slice(domain[2],domain[3]))
                            lat = bdata['lat']
                            bdata, lon = add_cyclic_point(bdata, coord=bdata['lon'])
                            lons, lats = np.meshgrid(lon, lat)
                            # determine levels & color normalization:
                            ...
                            if key == "m_data":
                                c = 0
                                # Plot to the correct subplot
                                cf = ax.contourf(lons, lats, adata, levels=levels, cmap=cmap,
                                            norm=norm)
                            if key == "o_data":
                                c = 1
                                # Plot to the correct subplot
                                cf = ax.contourf(lons, lats, bdata, levels=levels, cmap=cmap,
                                            norm=norm)
                        ...
                        if c == 0:
                            ...
                            cbar.ax.tick_params(labelsize=6) # Adjust the size as needed
                        # End if
            ...
            if (not redo_plot) and Path(wks / file_name).is_file():
                ...
                for r in range(nrows):
                    ...
                    for c, key in enumerate(["m_data", "o_data", "diff_data"]):
                        ...
                        #downsize to the specified region; makes plotting/rendering/saving much faster
                        fld = fld.sel(lat=slice(domain[2],domain[3]))
                        mean_val = fld.weighted(wgt).mean().item()
                        max_val  = fld.max()
                        min_val  = fld.min()
                        ...
                        lons, lats = plot_utils.transform_coordinates_for_projection(proj, lon_cyclic, fld.lat) # Explicit coordinate transform
